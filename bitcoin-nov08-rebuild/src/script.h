...
enum opcodetype
{
    ...
    OP_SHA1,
    OP_SHA256,
    // nov08-rebuild: OP_HASH160/OP_HASH256 did NOT exist in nov08. In 0.1.0 they
    // were inserted here, shifting OP_CODESEPARATOR 0xa9->0xab and OP_CHECKSIG
    // 0xaa->0xac. nov08 had OP_CODESEPARATOR=0xa9 and OP_CHECKSIG=0xaa, which is
    // required to reproduce the genesis merkle root byte-for-byte. We keep the
    // names but move them to later values so script.cpp still compiles and
    // EvalScript stays self-consistent.
    OP_CODESEPARATOR,        // 0xa9 (nov08)
    OP_CHECKSIG,             // 0xaa (nov08)
    OP_CHECKSIGVERIFY,
    OP_CHECKMULTISIG,
...
// nov08: signs by POSITION of the spent output (adds the input to txTo and signs it)
bool SignSignature(const CTransaction& txFrom, int nOut, CTransaction& txTo, int nHashType=SIGHASH_ALL, const char* pszType="all");
