...
uint256 SignatureHash(CScript scriptCode, const CTransaction& txTo, unsigned int nIn, int nHashType)
{
    ...
    if ((nHashType & 0x1f) == SIGHASH_NONE)
    {
        ...
        // nov08: CTxIn has no nSequence (it lives in CTxOut), so the "let the
        // others update at will" mechanism via input sequence does not exist.
    }
    else if ((nHashType & 0x1f) == SIGHASH_SINGLE)
    {
        ...
        for (int i = 0; i < nOut; i++)
            txTmp.vout[i].nSequence = UINT_MAX;  // nov08: CTxOut::IsNull() <=> nSequence==UINT_MAX
        // nov08: see the note above about input nSequence
    }
...
// nov08: identifies the output to spend by POSITION (nOut) in txFrom. Adds a new
// input to txTo pointing at txFrom.vout[nOut] and signs it with SIGHASH_ALL.
bool SignSignature(const CTransaction& txFrom, int nOut, CTransaction& txTo, int nHashType, const char* pszType)
{
    assert(nOut >= 0 && nOut < (int)txFrom.vout.size());
    const CTxOut& txout = txFrom.vout[nOut];

    // Add the input that spends this output
    txTo.vin.push_back(CTxIn(txFrom.GetHash(), nOut, CScript()));
    unsigned int nIn = txTo.vin.size() - 1;
    CTxIn& txin = txTo.vin[nIn];

    // Sign (the signature is not included in the hash being signed)
    uint256 hash = SignatureHash(txout.scriptPubKey, txTo, nIn, SIGHASH_ALL);
    if (!Solver(txout.scriptPubKey, hash, SIGHASH_ALL, txin.scriptSig))
        return false;
